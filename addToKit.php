...
echo "<script>userCheck('".$pType."','".$page."');</script>";
    ...
    $stmt = $conn->prepare("SELECT * FROM KitCart WHERE Account_Number = '$userID' AND Part_Number = '$part_number'; "); 
    $stmt->execute();
    ...
    if(count($result)>0)
    {
      ...
      $qty = 1+ $qty;
      $sql = "UPDATE KitCart SET Quantity = '$qty' WHERE Part_Number = '$part_number' AND Account_Number = '$userID' ;";
    }else{
    $sql = "insert into KitCart values ('$part_number','$part_name','$quantity','$description','$userID')";
    }
