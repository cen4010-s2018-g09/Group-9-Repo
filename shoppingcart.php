...
session_start();
      ...
      try{
        ...
        $stmt = $conn->prepare("SELECT * FROM ShoppingCart WHERE Account_Number = '$userID';"); 
        $stmt->execute();
...
<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top fixed-top-2">
    ...
    <div >
        ...
        <div class="row">
            ...
            <div class="column"><button class="btn btn-warning btn-sm" onclick="CheckOut();" name="CheckOut" >Check Out</button></div>
        </div>
      ...
      try{
        ...
        if(count($result)>0)
        {
            ...
            for($i=0;$i<count($result);$i++)
            {
                ...
                echo 'displayCartItems("'.$part_number.'","'.$part_name.'","'.$quantity.'","'.$description.'");';
