...
<?php
      ...
      try{
        ...
        if(count($result)>0)
        {
            ...
            for($i=0;$i<count($result);$i++)
            {
                ...
                $_SESSION["part_name"] = $row['Part Name'];
                $_SESSION["vendor"] = $row['Vendor P/N'];
                $_SESSION["price"] = $row['Price'];
                $_SESSION["quantity"] = $row['Quantity'];
                $_SESSION["description"] = $row['Description'];
                $_SESSION["location"] = $row['Location in Lab'];
              }
  ...
  $location =$_SESSION["location"];
      ...
      echo 'displayItemDetails("'.$part_number.'","'.$part_name.'","'.$vendor.'","'.$price.'","'.$quantity.'","'.$description.'", "'.$location.'");';
      if ($profile_type == "Admin" || $profile_type == "Staff"){
        echo 'addEditSection("'.$price.'","'.$part_number.'","'.$vendor.'","'.$quantity.'");';
      }
    ?>
    </script>
</html>
